...
def detect_edge_points(all_p, k1, k2):
    ...
    neighbors_idx_2 = getNeighbors(all_p, all_p, k2, False)
    lambda_ = 1.45
    thresh = 0.01
    ...
    for i, x in enumerate(all_p):
        ...
        e = np.sort(np.linalg.eig(np.cov(all_p[neighbors_idx_2[i]].T))[0])
        weights.append([z, dist, e[0] / (e[0] + e[1] + e[2])])
...
def simplify_cube(points, n_e_points,e_points, range_):
    ...
    print(points.shape[0],n_e_points.shape[0],e_points.shape[0])
